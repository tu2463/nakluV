/* cube-main.cpp (Downloads) is a GPU-compute skeleton for the same job your existing main-cube.cpp already does on the CPU — bake a radiance cubemap into a Lambertian irradiance cubemap, but using a Vulkan compute shader
  (CubePipeline) instead of a CPU loop.
  Your current main-cube.cpp is a complete, correct CPU implementation. The cube-main.cpp is a GPU-accelerated version — faster for large cubemaps, but significantly more Vulkan boilerplate.*/
#include "RTG.hpp"
#include "VK.hpp"
...
//   Section: WORLD_FROM_PX
//   What it does: A 3×3 matrix mapping pixel coords (px, py) → world-space direction. Built from s (right tangent), t (down tangent), and center (face center direction). The shader uses this to reconstruct the ray direction for
/*
### main
