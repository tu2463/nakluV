...
static void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
	...
	// event.key.key = uint8_t(key); // FIXED-BUG: truncates to 8 bits. e.g. the tab key is 258 (100000010), which will be truncated to 2 (00000010), causing it to not match with GLFW_KEY_TAB
	event.key.key = key;
