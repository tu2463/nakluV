...
struct Helpers {
	...
	// If data == nullptr, skips the buffer copy and only performs the layout transition (useful for depth images).
	void transfer_to_image(void const *data, size_t size, AllocatedImage &image, uint32_t layerCount = 1, VkImageLayout final_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, uint32_t mip_level = 0, VkImageAspectFlags aspect = VK_IMAGE_ASPECT_COLOR_BIT);
