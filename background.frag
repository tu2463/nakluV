...
void main() {
    ...
    float frequency1 = 1.5;
    float frequency2 = 1.5;

    float amplitude1 = 0.6;
    ...
    if (time < t1) {
        ...
        //wave1 = wave1_y + sin((uv.x + (time - t1) * ( speed + ( 0.01 - min(t1 - time, 0) ) )) * frequency1 * 3.14159) * amplitude1;
        wave1 = wave1_y - sin(pos_offset + (uv.x + (time - t1) * (speed + ds1)) * frequency1 * 3.14159) * amplitude1;
        wave2 = wave2_y - sin(pos_offset + (uv.x + (time - t1) * (speed + ds2)) * frequency2 * 3.14159) * amplitude2;
    }
    ...
    vec3 waveColor1 = vec3(0.2, 0.8, 1.0);  // Cyan for top wave
    vec3 waveColor2 = vec3(0.4, 0.6, 1.0);  // light blue for bottom wave
    vec3 backgroundColor = vec3(0.025, 0.025, 0.05);  // Dark background
